...
def try_snmp_objects_properties(snmp_client, cls, properties, subindex=None, timeout=4, retries=5, snmp_objects=None):
    ...
    for field, field_property in as_tuples(properties):
        ...
        fill_snmp_objects(snmp_objects, walk_datas, cls, field, field_property)
        # logger.info("[SNMP] try_snmp_objects4 snmp_objects %d", len(snmp_objects))

    return snmp_objects


def fill_snmp_objects(snmp_objects, walk_datas, cls, field, field_property):
    # logger.info("[SNMP] fill_snmp_objects walk_datas:%d snmp_objects:%d", len(walk_datas), len(snmp_objects))
    loop_index = 0
    for walk_index, walk_data in walk_datas:
        loop_index+=1
        # logger.info("[SNMP] fill_snmp_objects loop (%d)", loop_index)
        # logger.info("[SNMP] fill_snmp_objects walk_index:%s walk_data:%d snmp_objects:%d", walk_index, len(walk_data), len(snmp_objects))
        current_index = None
        current_subindex = None
        o = None
        for index, snmp_object in snmp_objects:
            # logger.info("[SNMP] fill_snmp_objects snmp_object:%s", snmp_object)
            if walk_index == index:
                o = snmp_object
                current_index = index
        ...
        # logger.info("[SNMP-FIXME] fill_snmp_objects and...")
        try:
            data_to_set = walk_data.itervalues().next()
        except Exception, exc:
            logger.warning("[SNMP-FIXME] Exception walk_data:%s", walk_data)
            data_to_set = None
        ...
        if field_property.method == 'get':
            ...
            if current_subindex:
                ...
                logger.warning("[SNMP-FIXME] no current_subindex (%s/%s/%s)...", walk_index, walk_data, data_to_set)
    # logger.info("[SNMP] fill_snmp_objects!!!")
